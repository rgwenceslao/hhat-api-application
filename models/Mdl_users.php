...
class mdl_users extends CI_Model {
	...
	{		
		...
		if($where) $this->db->where($where, NULL, FALSE);
		$this->db->select("id,name,phone,address, DATE_FORMAT(expiration,'%b, %d %Y') as expiration", FALSE);
	...
	public function api_sendmessage($data)
	{
		...
		$mail['subject'] 	= 'Message from hand hygiene app.';
		$mail['to'] 		= 'hhat@bbraun.com';
		$mail['from'] 		= array('email' => 'noreply@bbraun.com','name' => $data->name);
		if($this->site->sendmail($mail))
			$result = array( 'status' => 1, 'errortype' => 'success', 'message' => 'Your message successfully sent.', 'result' => '');
		else
